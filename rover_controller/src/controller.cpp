...
namespace rover_controller
{
  ...
  bool Controller::initialize(ros::NodeHandle& nh)
  {
    ...
    command_pub_ = nh.advertise<geometry_msgs::Twist>("/base_velocity",1);//#>>>>TODO: advertise the topic

    field_pub_ = nh.advertise<geometry_msgs::PoseArray>("/field",1);//#>>>>TODO: advertise a geometry_msgs::PoseArray with name "/field"
    ...
    goal_sub_ = nh.subscribe("/move_base_simple/goal", 1, 
                             &Controller::goalCallback/*#>>>>TODO: The NAME of the callback function*/,
                             this);

    //#>>>>TODO: Subscribe to the obstacles msg published by the package object_server                     
    obstacles_sub_ = nh.subscribe("/obstacles",/*#>>>>TODO: The NAME of the command topic*/ 1, 
                             &Controller::obstacleCallback/*#>>>>TODO: The NAME of the callback function*/, this);
    ...
    if(!ros::param::get("/attractor_gain"/*#>>>>TODO: PARAMETER NAME*/, v))
      return false;
    K_attractor_ = Eigen::Vector3d(v[0], v[1], v[2]).asDiagonal();

    if(!ros::param::get("/repulsive_gain"/*#>>>>TODO: PARAMETER NAME*/, d))
      return false;
    k_repulsive_ = d;

    if(!ros::param::get("/lambda_repulsive"/*#>>>>TODO: PARAMETER NAME*/, d))
      return false;
    lambda_repulsive_ = d;
  
    if(!ros::param::get("/vortex_gain"/*#>>>>TODO: PARAMETER NAME*/, d))
      return false;
    k_vortex_ = d;

    if(!ros::param::get("/lambda_vortex"/*#>>>>TODO: PARAMETER NAME*/, d))
      return false;
    lambda_vortex_ = d;
    ...
    const Eigen::Vector3d& goal, const Eigen::Vector3d& pos)
  {
    ...
    return K_attractor_*e;//#Eigen::Vector3d::Zero(); //#>>>>TODO: Replace
  }
    ...
    std::vector<Obstacle> obstacles, const Eigen::Vector3d& pos)
  {
    ...
    {
      v <<  (obstacles[i].pos - pos).head(2), 0; //#obstacles[i].pos-pos; v.z()=0;//#>>>>TODO: 2d distance of obstacle and rover (note z component is zero!)
      ...
      double f = k_repulsive_*std::exp(-lambda_repulsive_*d);//#>>>>TODO: force magnitude
    
      force -= f*v.normalized();//#>>>>TODO: add up the repulsive force vector for this obstacle
    }
    return force;
  }
    ...
    std::vector<Obstacle> obstacles, const Eigen::Vector3d& pos)
  {
    ...
    for(size_t i = 0; i < obstacles.size(); ++i)
    {
      ...
      { 
        ...
        R.topLeftCorner(2,2) = Eigen::Rotation2Dd(-M_PI/2).toRotationMatrix();//#>>>>TODO: 
      }
      else
      {
        //#>>>>TODO: set the upper left 2x2 corner of the rotation matrix R
  ...
  Eigen::Vector3d Controller::roverPos()
  {
    ...
    try {
      listener_.lookupTransform(
        ...
        "base_footprint"/*#>>>>TODO: the source frame name*/, ros::Time(0), transform);
  ...
  void Controller::goalCallback(const geometry_msgs::PoseStampedConstPtr& msg)
  {
    ...
    ROS_INFO_STREAM("goal vector= "<<goal_pos_w_);
  }
  ...
  void Controller::obstacleCallback(const object_msgs::ObjectsConstPtr& msg)
  {
    ...
    for(size_t i = 0; i < msg->objects.size(); ++i)
    {
      ...
      ROS_INFO_STREAM_THROTTLE(0.5, "obstacles position=" << obstacles_[i].pos);
    }
  }
